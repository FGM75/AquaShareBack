...
const serviceAccount = require("../../aquashare-94e77-firebase-adminsdk-omjde-55837a3fbc.json"); // JSON descargado desde Firebase
...
if (!admin.apps.length) {
  admin.initializeApp({
    ...
    storageBucket: "gs://aquashare-94e77.appspot.com", // Nombre de vuestro bucket
  });
} else {
...
router.post("/nuevo-punto", upload.single("imagen"), async (req, res, next) => {
  ...
  try {
    ...
    const ficheroFB = datos.createWriteStream({ metadata: {
      contentType: req.file.mimetype,
      metadata: {
        firebaseStorageDownloadTokens: Date.now() + '_' + Math.random().toString(36).substring(2)
      }
    }});
    ...
    ficheroFB.on("finish", async () => {
      ...
      parameters.urls.push(
        `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(datos.name)}?alt=media&token=${datos.metadata.metadata.firebaseStorageDownloadTokens}`
      );
